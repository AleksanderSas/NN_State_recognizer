...
int _main(int argc, char *argv[], int* sample_no, float** data)
{
  ...
  if (NumArgs() == 0);
      ...
      //while (NumArgs() > 0 ) {
         ...
         ListSpeech(argv[1], sample_no, data);
      //}
   //Exit(0);
...
void ListParms(char *src, int* sample_no, float** data)
{
   ...
   *sample_no = ho.nSamples;
   *data = malloc(sizeof(float) * ho.nSamples * 39);
   ...
   if (prData || gst>-1 || gen >-1) {
      ...
      while (BufferStatus(pbuf) != PB_CLEARED){
         ...
         if (idx >= gst  && (idx <= gen || gen==-1)){
            ...
            {
               PrintObservation((int)idx, &o, nItems, (*data) + 39 * current_sample_no);
               current_sample_no++;
            }
            fflush(stdout);
         }
...
void ListSpeech(char *src, int* sample_no, float** data)
{
   //if (IsWave(src)){
      //if (src==NULL)
        // ListWavefromAudio();
