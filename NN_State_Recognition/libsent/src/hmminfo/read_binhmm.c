...
read_binhmm(FILE *fp, HTK_HMM_INFO *hmm, boolean gzfile_p, Value *para)
{
  ...
  /* Dump the structure of macro models */
  model_ptr = hmm->start;
  s_index = 0;
  while ( model_ptr != NULL )
  {	
	 if (hmm->is_outprob)
     for ( si = 0; si < model_ptr->state_num; si++ )
     {
        if ( model_ptr->s[si] != NULL )
        {
           if (s_index >= MAX_NUM_STATES)
           {
              jlog("Error: out of boundary while state reordering\n");
              return (FALSE);
           }
           hmm->state_order[s_index] = model_ptr->s[si]->id;
           s_index++;           
/*
           if (  ((model_ptr->s)[si])->name == NULL )
              printf( "   state %d: NULL   id: %d\n", si, (model_ptr->s[si])->id );
           else
              printf( "   state %d: %s   id: %d\n", si, (model_ptr->s[si])->name, (model_ptr->s[si])->id );
*/
        }
     }
