...
read_param(FILE *fp, HTK_Param *pinfo)
{
  ...
  printf("SAMPTYPE READ TO %d\n", (int)(hd->samptype  & F_BASEMASK ));
  // JSAS - if read number is 11 (PLP) then convert it to unknown
  ...
  for (i = 0; i < pinfo->samplenum; i++) {
	  if (hd->samptype & F_COMPRESS) {
		  ...
		  for (v = 0; v < pinfo->veclen; v++) {
			  ...
			  if (pinfo->is_outprob)
			  {
				  // printf( "input elem %d goes to state id %d\n", v,  pinfo->state_order[v] );
				  pinfo->parvec[i][pinfo->state_order[v]] = d;
			  }
			  else
				  pinfo->parvec[i][v] = d;
			  p += sizeof(float);
